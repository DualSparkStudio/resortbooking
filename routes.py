...
def guest_details():
    ...
    if form.validate_on_submit():
        ...
        # Store booking ID and redirect to payment selection
...
def payment_selection():
    booking_id = session.get('pending_payment_booking_id')
    if not booking_id:
        flash('No pending booking found. Please start the booking process again.', 'warning')
        return redirect(url_for('index'))
...
@app.route('/payment/test/<int:booking_id>')
def process_test_payment(booking_id):
    """Test payment route that bypasses actual payment processing"""
    booking = Booking.query.get_or_404(booking_id)
    
    # Check if this is the user's pending booking
    if session.get('pending_payment_booking_id') != booking_id:
    ...
    try:
        ...
        # Clear session
        session.pop('pending_payment_booking_id', None)
        
        flash('Test payment successful! Your booking has been confirmed.', 'success')
        return redirect(url_for('booking_success_fallback', booking_id=booking.id))
        
    except Exception as e:
        app.logger.error(f"Error in test payment: {str(e)}")
        flash('Test payment failed. Please try again.', 'danger')
        return redirect(url_for('payment_selection'))


@app.route('/payment/stripe/<int:booking_id>')
def process_stripe_payment(booking_id):
    ...
    if not stripe_secret_key or not stripe_publishable_key:
        flash('Stripe payment is not configured.', 'danger')
        return redirect(url_for('payment_selection'))
    
    session_data = create_stripe_checkout_session(booking, stripe_secret_key, stripe_publishable_key)
    if session_data:
...
@app.route('/payment/razorpay/<int:booking_id>')
def process_razorpay_payment(booking_id):
    booking = Booking.query.get_or_404(booking_id)
...
def verify_razorpay_payment_route():
    try:
        ...
        app.logger.info("=== Razorpay Payment Verification Started ===")
        ...
        app.logger.info(f"Signature present: {'Yes' if signature else 'No'}")
        
        if not all([payment_id, order_id, signature]):
            app.logger.error("Missing payment data - payment_id, order_id, or signature is None")
        ...
        if not booking:
            app.logger.error(f"No booking found with Razorpay order ID: {order_id}")
            flash('Booking not found.', 'danger')
            return redirect(url_for('index'))
        
        app.logger.info(f"Found booking ID: {booking.id} for order: {order_id}")
        
        # Verify payment
        app.logger.info("Starting payment signature verification...")
        verification_result = verify_razorpay_payment(payment_id, order_id, signature)
        app.logger.info(f"Payment verification result: {verification_result}")
        
        if verification_result:
            app.logger.info("Payment verification successful - updating booking status")
            # Update booking status
            booking.payment_status = 'paid'
            booking.booking_status = 'confirmed'
            booking.razorpay_payment_id = payment_id
            db.session.commit()
            app.logger.info(f"Booking {booking.id} status updated to confirmed/paid")
            ...
            app.logger.error("Payment verification failed - signature mismatch or invalid payment")
            flash('Payment verification failed. Please contact support.', 'danger')
            return redirect(url_for('payment_selection'))
            
    except Exception as e:
        app.logger.error(f"Exception in Razorpay payment verification: {str(e)}")
...
def admin_add_room_type():
    ...
    if request.method == 'POST':
        ...
        try:
            # Process amenities - split by newlines and filter empty lines
            amenities_text = request.form.get('amenities', '')
            if amenities_text:
                amenities_list = [line.strip() for line in amenities_text.split('\n') if line.strip()]
                processed_amenities = '\n'.join(amenities_list)
            else:
                processed_amenities = ''
            ...
            for i, file in enumerate(uploaded_files):
                if file and file.filename:
                    try:
                        # Save the file using utility function
                        image_url = save_uploaded_image(file, app.config['UPLOAD_FOLDER'], f"room_type_{room_type.id}")
...
def admin_edit_room_type(id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            app.logger.debug(f"Files: {list(request.files.keys())}")
            ...
            app.logger.debug(f"Number of uploaded files: {len(uploaded_files)}")
            for i, file in enumerate(uploaded_files):
                if file and file.filename:
                    app.logger.debug(f"  File {i}: {file.filename} ({file.content_length} bytes)")
            
            # Log new URLs
            new_image_urls = request.form.getlist('new_image_urls[]')
            app.logger.debug(f"Number of new URLs: {len(new_image_urls)}")
            for i, url in enumerate(new_image_urls):
                if url.strip():
                    app.logger.debug(f"  URL {i}: {url}")
            ...
            keep_images = request.form.get('keep_images', '').split(',')
            ...
            db.session.rollback()
            app.logger.error(f"Error updating room type: {str(e)}")
            app.logger.error(f"Exception type: {type(e).__name__}")
            import traceback
